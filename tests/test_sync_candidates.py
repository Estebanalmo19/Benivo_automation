...
def test_upsert_sql_preserves_existing_is_vip_true_on_conflict():
    ...
    do_update_clause = sync_candidates._UPSERT_SQL.split("DO UPDATE SET")[1]
...
def test_upsert_sql_omits_is_vip_from_insert_so_new_rows_get_the_column_default():
    ...
    insert_columns = sync_candidates._UPSERT_SQL.split("INSERT INTO benivo.candidates (")[1].split(")")[0]
...
def test_delete_out_of_scope_uses_same_eligibility_filter_as_upsert():
    upsert_sql = sync_candidates._UPSERT_SQL
    delete_sql = sync_candidates._DELETE_OUT_OF_SCOPE_SQL
